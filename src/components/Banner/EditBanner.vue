...
export default {
    created: function() {
        ...
        const data = this.$store.getters.getBanner.filter( function(item) {
            return item.id === id
        });
        ...
        this.data.sequence = data[0].sequence;
        this.data.funds_available = data[0].funds_available;
        this.data.status = data[0].status === "active" ? true : false
    },
    data: function() {
        return {
            data: {
    ...
    methods: {
        ...
        send: async function() {
            ...
            try {
                let result = await this.server.patch("update_banner", {
                    ...this.data,
